...
window.renderSidebar = function() {
  ...
  container.innerHTML = `
    ...
    ">
      ...
      ` : `
        <!-- Divider + Workspaces -->
        <div style="padding: 12px 16px 6px; border-top: 1px solid var(--border-subtle); margin-top: 8px; flex-shrink:0;">
          <div style="font-size:11px; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.08em; font-weight:600; margin-bottom:6px;">Workspaces</div>
          <div id="workspace-list">
            ${workspaces.length === 0
              ? `<div style="font-size:12px; color:var(--text-muted); padding: 4px 10px;">No workspaces yet</div>`
              : workspaces.map(ws => `
                <div onclick="window._workspaceClick('${ws.name}')" style="
                  display:flex; align-items:center; gap:8px;
                  padding: 6px 10px; border-radius: 6px; cursor: pointer;
                  font-size: 13px; transition: background 0.1s;
                  color: var(--text-secondary);
                " onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background='transparent'">
                  <div style="width:6px; height:6px; border-radius:50%; background:${ws.color || 'var(--accent)'}; flex-shrink:0;"></div>
                  <span>${ws.name}</span>
                </div>
              `).join('')
            }
